...
const deleteOwnChirpByID = `-- name: DeleteOwnChirpByID :many
DELETE FROM chirps WHERE id = $1 AND user_id = $2 RETURNING id, created_at, updated_at, body, user_id
`

type DeleteOwnChirpByIDParams struct {
	ID     uuid.UUID `json:"id"`
	UserID uuid.UUID `json:"user_id"`
}

func (q *Queries) DeleteOwnChirpByID(ctx context.Context, arg DeleteOwnChirpByIDParams) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, deleteOwnChirpByID, arg.ID, arg.UserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) GetChirpsByUserID(ctx context.Context, arg GetChirpsByUserIDParams) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getChirpsByUserID, arg.UserID, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
